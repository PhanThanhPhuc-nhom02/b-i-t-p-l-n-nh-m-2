...
public:
    ...
    double tinhTien(time_t ra) const { return laQuaNgay(timestampVao, ra) ? 6000 : 3000; }
};

class XeGa : public PhuongTien {
public:
    XeGa() { loaiXe = "Xe May (Ga)"; }
    double tinhTien(time_t ra) const { return laQuaNgay(timestampVao, ra) ? 8000 : 4000; }
};

class OTo : public PhuongTien {
public:
    OTo() { loaiXe = "Xe O To"; }
    double tinhTien(time_t ra) const { return laQuaNgay(timestampVao, ra) ? 40000 : 20000; }
};

class XeTai : public PhuongTien {
public:
    XeTai() { loaiXe = "Xe Tai"; }
    double tinhTien(time_t ra) const { return laQuaNgay(timestampVao, ra) ? 90000 : 45000; }
};

class XeDap : public PhuongTien {
public:
    XeDap() { loaiXe = "Xe Dap"; }
    double tinhTien(time_t ra) const { return laQuaNgay(timestampVao, ra) ? 4000 : 2000; }
};
...
public:
    MoHinhBaiXe(int sc) : SUC_CHUA(sc), doanhThu(0), tongXeDaVao(0) {}
    ~MoHinhBaiXe() { for (size_t i = 0; i < danhSach.size(); i++) delete danhSach[i]; }
    ...
    void hienThiLichSu() const {
        ...
        if (dsLichSu.empty()) cout << " (Chua co lich su giao dich)\n";
